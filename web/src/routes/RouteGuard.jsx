...
const RouteGuard = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!matchRoute) {
      ...
      if (matchRoute.auth && !GetToken()) { // 需要登录但是未登录
        navigator('/login');
      } else if (matchRoute.auth) { // 需要登录且已登录，则校验 Token 是否过期
        AxiosGet(Apis.Public.TokenVerification).then((res) => {
          if (res.code !== 200) {
            navigator('/login');
          }
        });
      } else if (location.pathname === '/login' && GetToken()) { // 登录页面还有 Token，则清理 localStorage
        localStorage.clear();
      }
